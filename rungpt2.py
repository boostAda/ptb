...
def perturb_texts_(texts, span_length, pct, ceil_pct=False):
    if not args.random_fills:
        ...
        # modification:print the results
        #print("masked_texts[0]:"+masked_texts[0] + "**\n")
        raw_fills = replace_masks(masked_texts)
        #print("raw_fills[0]" + raw_fills[0] + "**\n")
        extracted_fills = extract_fills(raw_fills)
        #print("extracted_fills" + ' '.join(extracted_fills[0]) + "**\n")
        perturbed_texts = apply_extracted_fills(masked_texts, extracted_fills)
        #print("perturbed_texts" + perturbed_texts[0] + "**\n")
        #modification:delete the while loop
        ...
        while '' in perturbed_texts:
            ...
            attempts += 1
        # tmp_lst = [tmp for tmp in perturbed_texts if tmp != '']
        # perturbed_texts = tmp_lst
    else:
...
#modifications: add acc and f1 as additional metircs
def get_precision_recall_metrics(real_preds, sample_preds):
    precision, recall, _ = precision_recall_curve([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds)
    pr_auc = auc(recall, precision)
    acc = accuracy_score([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds)
    f1 = f1_score([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds)
    return precision.tolist(), recall.tolist(), float(pr_auc), acc.tolist(), f1.tolist()
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500):
    ...
    #modification : change len(original_text) into min(len(original_text), len(sampled_text))
    for idx in range(min(len(original_text), len(sampled_text))):
        results.append({
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500):
    ...
    p, r, pr_auc, acc, f1 = get_precision_recall_metrics(predictions['real'], predictions['samples'])
...
def run_baseline_threshold_experiment(criterion_fn, name, n_samples=500):
    ...
    ### modification for idx in range(len(original_text)):       changed into   range(min(len(original_text), len(sampled_text))):
        for idx in range(min(len(original_text), len(sampled_text))):
            results.append({
                "original": original_text[idx],
...
def generate_data(dataset, key):
    ...
    if dataset in custom_datasets.DATASETS:
        #data = custom_datasets.load(dataset, cache_dir, args.datadir)load_Imdb
        data = custom_datasets.load_Imdb(args.datadir, cache_dir)
...
def generate_stega_data(dataset, key):
    if dataset in custom_datasets.DATASETS:
        data_cover, data_stego = custom_datasets.load_xsum(args.datadir, cache_dir)
    else:
        data = datasets.load_dataset(dataset, split='train', cache_dir=cache_dir)[key]
...
def stegatext_process(data):
    ...
    long_data = [x for x in data if len(x.split()) >200 and len(x.split()) < 300]
    if len(long_data) > 0:
        data = long_data
    # keep only examples with <= 512 tokens according to mask_tokenizer
    # this step has the extra effect of removing examples with low-quality/garbage content
...
def eval_supervised(data, model):
    ...
    p, r, pr_auc, acc, f1 = get_precision_recall_metrics(real_preds, fake_preds)
...
if __name__ == '__main__':
    ...
    #modifications: 运行时改为--pct_words_masked=0.1
    parser.add_argument('--pct_words_masked', type=float, default=0.1) # pct masked is actually pct_words_masked * (span_length / (span_length + 2 * buffer_size))
    parser.add_argument('--span_length', type=int, default=2)
    parser.add_argument('--n_samples', type=int, default=200)
    parser.add_argument('--n_perturbation_list', type=str, default="10,20,50,100")
    ...
    parser.add_argument('--output_name', type=str, default="hc")
    parser.add_argument('--openai_model', type=str, default=None)
    parser.add_argument('--openai_key', type=str)
    parser.add_argument('--baselines_only', default=True)
    # modification: default='True'
    parser.add_argument('--skip_baselines',  default=False)
    parser.add_argument('--buffer_size', type=int, default=1)
    parser.add_argument('--mask_top_p', type=float, default=1.0)
    parser.add_argument('--pre_perturb_pct', type=float, default=0.0)
    parser.add_argument('--pre_perturb_span_length', type=int, default=5)
    #modification: default='True'
    parser.add_argument('--random_fills', action='store_true', default='')
    parser.add_argument('--random_fills_tokens', action='store_true', default='False')
    ...
    SAVE_FOLDER = f"tmp_results/{output_subfolder}{base_model_name}{scoring_model_string}-{args.mask_filling_model_name}-{sampling_string}/{START_DATE}-{START_TIME}-{precision_string}-{args.pct_words_masked}-{args.n_perturbation_rounds}-{args.dataset}-{args.n_samples}--{args.datadir}"
    ...
    if not args.baselines_only and not args.random_fills:
        ...
        #mask_model = T5ForConditionalGeneration.from_pretrained(mask_filling_model_name)
        try:
    ...
    #mask_tokenizer = T5Tokenizer.from_pretrained(mask_filling_model_name, model_max_length=n_positions, cache_dir=cache_dir)
    ...
    data = generate_stega_data(args.dataset, args.dataset_key)
    ...
    if not args.skip_baselines:
        ...
        #modifications: delete the following two lines of code:
